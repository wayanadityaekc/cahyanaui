...
// Capped but NOT centred. The SECTION owns the margin now (PAGE_WIDE), so the row
// has to start at the page's left edge - `mx-auto` would float it back into the
// middle of the wider box and put the guide page back on two left edges.
// The cap stays because auto-fill sizes columns from the container: letting the row
// span the full 1752px made the same cards render 291px @1280, 260 @1440 and 273
// @1920 (measured), so they shrank as the screen grew and stopped matching the
// homepage cards. The cap is the homepage GRID width written the way `.catsec`
// writes it (container minus its two gutters = 1152), which is what makes a card
// land at 271px - the same number it has on the homepage - at every desktop width.
// The white to their right is the same compromise the 720px prose above them makes.
