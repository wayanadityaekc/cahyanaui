// Card-grid / slider engine, ported from CUE's components/ui/gridClasses.js.
// One self-contained utility string per context: the `[&>*]:` child sizing does
// the job the old `.experience__grid > *` direct-child CSS rules did, and the
// breakpoints mirror CUE's @media exactly, so a card row on this site and the
// same row on cahyanaubudexperience.com land on the same geometry.
//
// Only the context-neutral tracks are carried over. CUE's page-specific ones
// (GRID_XPLORE / GRID_GUIDEHUB / GRID_GUIDEMORE / GRID_RELATED) are scoped to
// sections that do not exist here; if a villa page ever needs one, copy it then
// rather than parking a dead string in this file.
...
// Section container for a card row: centred, uniform side padding via the
