...
export default function ContactForm({ company = false }) {
  ...
  // React Hook Form owns the field values and the per-field messages; the schema
  // is the same one the rules test covers. Messages appear on the first submit
  // and then clear as soon as a field becomes valid (RHF's default re-validation).
  const {
    register,
    handleSubmit,
  ...
  // `data` is the schema's output, so it arrives already trimmed.
  ...
  return (
    <>
      ...
      <div className={CONTACT_GROUP}>
        ...
        <input className={CONTACT_INPUT} type="text" id="c-name" placeholder="Enter your name" {...register('name')} aria-invalid={!!errors.name} />
        {errors.name && <small className={FIELD_ERR}>{errors.name.message}</small>}
      </div>
      <div className={CONTACT_GROUP}>
        <label className={CONTACT_LABEL} htmlFor="c-email">Email</label>
        <input className={CONTACT_INPUT} type="email" id="c-email" placeholder="you@email.com" {...register('email')} aria-invalid={!!errors.email} />
        {errors.email && <small className={FIELD_ERR}>{errors.email.message}</small>}
      </div>
      <div className={CONTACT_GROUP}>
        <label className={CONTACT_LABEL} htmlFor="c-message">Message</label>
        <textarea className={CONTACT_TEXTAREA} id="c-message" placeholder="Tell us what you need - dates, group size, custom requests..." {...register('message')} aria-invalid={!!errors.message} />
        {errors.message && <small className={FIELD_ERR}>{errors.message.message}</small>}
      </div>
      {error && <small className={REFMSG_ERR}>{error}</small>}
        <button className="w-full p-[0.85rem] border-none rounded-pill text-[1rem] font-semibold text-white bg-cta cursor-pointer hover:bg-cta-d" id="c-send" onClick={send} disabled={isSubmitting}>
